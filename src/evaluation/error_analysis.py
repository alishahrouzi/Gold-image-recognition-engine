...
def select_representative_examples(
    ...
    *,
    ...
    """Select deterministic, useful examples without scanning arbitrary first rows."""
    if per_group < 1:
    ...
    for name, candidates in groups.items():
        if name == "high_similarity_wrong":
            ordered = sorted(candidates, key=lambda r: (-float(r.top1_candidate["similarity"]), r.query_id))
            selected = ordered[:per_group]
            ...
            rng.shuffle(pool)
            selected = sorted(pool[:per_group], key=_rank_key)
