...
def test_representative_selection_is_deterministic():
    ...
    for i in range(6):
        result = make_result()
        evaluation = QueryEvaluationRecord(**{**make_eval(first_rank=2, top1=0, top5=1, top10=1).to_dict(), "query_id": f"q{i}"})
        records.append(build_error_record(result, evaluation))

    first = select_representative_examples(records, per_group=3, seed=42)
    second = select_representative_examples(records, per_group=3, seed=42)
    assert first == second
    assert len(first["top1_wrong_top5_correct"]) == 3


def test_summary_counts_excluded_queries():
    records = [
