...
def _gallery() -> Gallery:
    # Deliberately non-tied similarities for the S3.8 search-engine smoke test.
...
def _write_gallery(
    ...
    _gallery().save(embedding_path, metadata_path)
...
def test_load_reads_embeddings_and_metadata_into_memory(tmp_path: Path) -> None:
    ...
    assert torch.allclose(
        torch.linalg.vector_norm(runtime.gallery.embeddings, dim=1),
        torch.ones(3),
    )
...
def test_loaded_search_engine_is_ready_for_product_search(tmp_path: Path) -> None:
    ...
    assert len({candidate.product_id for candidate in result.candidates}) == 2
...
def test_from_gallery_supports_in_memory_runtime(tmp_path: Path) -> None:
    ...
    assert runtime.search_engine.metadata == gallery.metadata
...
def test_empty_gallery_is_rejected() -> None:
    gallery = Gallery(embeddings=torch.empty((0, 3)), metadata=tuple())
    loader = GalleryLoader(
        GalleryRuntimeConfig(model_name="memory", expected_embedding_dim=3)
    )
...
def test_empty_product_id_is_rejected() -> None:
    gallery = Gallery(
        embeddings=torch.tensor([[1.0, 0.0]]),
        metadata=(_metadata("ring-1", "", "Ring"),),
    )
    loader = GalleryLoader(
        GalleryRuntimeConfig(model_name="memory", expected_embedding_dim=2)
    )
...
def test_empty_category_is_rejected() -> None:
    gallery = Gallery(
        embeddings=torch.tensor([[1.0, 0.0]]),
        metadata=(_metadata("ring-1", "p-ring", ""),),
    )
    loader = GalleryLoader(
        GalleryRuntimeConfig(model_name="memory", expected_embedding_dim=2)
    )
...
def test_empty_image_id_is_rejected() -> None:
    gallery = Gallery(
        embeddings=torch.tensor([[1.0, 0.0]]),
        metadata=(_metadata("", "p-ring", "Ring"),),
    )
    loader = GalleryLoader(
        GalleryRuntimeConfig(model_name="memory", expected_embedding_dim=2)
    )

    with pytest.raises(ValueError, match="empty image_id"):
        loader.from_gallery(gallery)
...
def test_legacy_metadata_is_normalized_and_accepted(tmp_path: Path) -> None:
    ...
    legacy_metadata = [
        {
            "product_group": "p-ring",
            "category": "Ring",
            "image_path": "ring-1.jpg",
            "image_id": "ring-1",
        }
    ]
    (directory / "gallery_metadata.json").write_text(
    ...
    runtime = GalleryLoader(
        GalleryRuntimeConfig(root=tmp_path, expected_embedding_dim=3)
    ).load()

    assert runtime.gallery.metadata[0]["product_id"] == "p-ring"
    assert runtime.gallery.metadata[0]["image"] == "ring-1.jpg"
    assert runtime.gallery.metadata[0]["product_group"] == "p-ring"
    assert runtime.gallery.metadata[0]["image_path"] == "ring-1.jpg"
